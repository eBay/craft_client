...
nuraft::ptr< nuraft::cluster_config > raft_state_mgr::load_config() {
    ...
    json config_map;
    auto reg = lock_registry(_registry_mgr);
    if (auto err = loadConfigFile(config_map, _group_id, _srv_id, reg); !err) { return fromClusterConfig(config_map); }
...
nuraft::ptr< nuraft::log_store > raft_state_mgr::load_log_store() {
    auto reg = lock_registry(_registry_mgr);
    auto log_store = reg->get< nuraft::inmem_log_store >(
...
nuraft::ptr< nuraft::srv_state > raft_state_mgr::read_state() {
    ...
    if (auto err = loadStateFile(state_map, _group_id, _srv_id, reg); !err) {
        try {
...
std::shared_ptr< nuraft::state_machine > raft_state_mgr::get_state_machine() {
    return std::make_shared< echo_state_machine >(_commit_cb, _group_id, lock_registry(_registry_mgr));
}
